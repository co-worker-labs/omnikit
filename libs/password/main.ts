...
export function maskPassword(type: PasswordType, password: string[]): string {
  const bullet = "\u2022";
  switch (type) {
    case "Random":
      return password[0].slice(0, 2) + bullet.repeat(Math.max(0, password[0].length - 2));
    case "Memorable":
      return password
        .map((w, i) =>
          i === 0
            ? w.slice(0, 2) + bullet.repeat(Math.max(0, w.length - 2))
            : bullet.repeat(w.length)
        )
...
function randomRangeChars(
  length: number,
  startChar: number,
  endChar: number,
  isAvoidAmibugous: boolean
): string {
  let result = "";
  for (var i = 0; i < length; i++) {
...
function generateRandom(characters: number, length: number): string {
  ...
  if (countArr[count_uppercase_index] > 0) {
    result += randomRangeChars(
      countArr[count_uppercase_index],
      "A".charCodeAt(0),
      "Z".charCodeAt(0),
      isAvoidAmibugous
    );
  ...
  if (countArr[count_lowercase_index] > 0) {
    result += randomRangeChars(
      countArr[count_lowercase_index],
      "a".charCodeAt(0),
      "z".charCodeAt(0),
      isAvoidAmibugous
    );
  ...
  if ((characters & memorable_capitalize_checked) > 0) {
    result = result.map((word) => {
      if (word.charAt(0) >= "a" && word.charAt(0) <= "z") {
        return String.fromCharCode(word.charCodeAt(0) - 32) + word.substring(1);
      }
      return word;
    });
  }
